...
CONFIG_PATH = "/home/app/config.json"
    ...
    def load_config(self) -> dict:
        default_config = {
            ...
            "tickets": {
                ...
                "categories": {"Technical": "🔧", "Moderation": "🛡️", "Other": "❓"},
                "active_tickets": {},
                "closed_tickets": {},
            },
            "moderation": {
            ...
            with open(CONFIG_PATH, "r") as f:
                try:
                    ...
                    if "events" not in config["logs"]["voice"]:
                        config["logs"]["voice"]["events"] = default_config["logs"][
                            "voice"
                        ]["events"]
                    else:
                    ...
                    if "events" not in config["logs"]["ticket"]:
                        config["logs"]["ticket"]["events"] = default_config["logs"][
                            "ticket"
                        ]["events"]
                    else:
                    ...
                    if "moderation" not in config:
                        ...
                        if "enabled" not in config["moderation"]:
                            config["moderation"]["enabled"] = default_config[
                                "moderation"
                            ]["enabled"]
                        if "banned_words" not in config["moderation"]:
                            config["moderation"]["banned_words"] = default_config[
                                "moderation"
                            ]["banned_words"]
...
async def on_ready():
    ...
    print(
        f"https://discord.com/api/oauth2/authorize?client_id={bot.application_id}&permissions=8&scope=bot%20applications.commands"
    )
    print("=" * 50)
