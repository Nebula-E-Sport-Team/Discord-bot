    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not role:
            await interaction.response.send_message(
                f"❌ Role with ID {self.role_id} no longer exists.",
                ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            f"✅ Role {role.mention} {action} you!",
            ephemeral=True
        )
...
class AutoroleSetupView(discord.ui.View):
    ...
    def update_buttons(self):
        for child in self.children:
            if isinstance(child, discord.ui.Button) and child.custom_id == "toggle":
                child.style = discord.ButtonStyle.green if self.bot.config["autorole"]["enabled"] else discord.ButtonStyle.red
    ...
    @discord.ui.button(label="Autorole: Disabled", style=discord.ButtonStyle.red, custom_id="toggle")
    async def toggle_autorole(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.bot.config["autorole"]["enabled"] = not self.bot.config["autorole"]["enabled"]
    ...
    async def update_role_message(self, interaction: discord.Interaction):
        channel = interaction.guild.get_channel(self.bot.config["autorole"]["channel_id"])
        if not channel:
            return
        ...
        try:
            ...
            if self.bot.config["autorole"]["message_id"]:
                try:
                    message = await channel.fetch_message(self.bot.config["autorole"]["message_id"])
...
class AutoroleChannelModal(discord.ui.Modal, title="Autorole Channel Setup"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(
                f"✅ Autorole channel set to {channel.mention}!",
                ephemeral=True
            )
        except ValueError:
...
class AddRoleModal(discord.ui.Modal, title="Add Autorole"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if not role:
                await interaction.response.send_message(
                    f"❌ Invalid role! ID: {role_id} not found.",
                    ephemeral=True
                )
                return
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if role_id not in self.bot.config["autorole"]["roles"]:
                await interaction.response.send_message("❌ This role is not in autoroles!", ephemeral=True)
                return
...
class Autorole(commands.Cog):
    ...
    def setup_views(self):
        ...
        if self.bot.config["autorole"].get("message_id"):
            view = RoleView(self.bot)
            self.bot.add_view(view, message_id=self.bot.config["autorole"]["message_id"])
