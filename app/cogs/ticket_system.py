...
class TicketSetupView(discord.ui.View):
    ...
    async def manage_categories(
        ...
        await interaction.response.edit_message(
            content="Ticket Categories Management", view=view
        )

    @discord.ui.button(
        label="Set Transcript Channel", style=discord.ButtonStyle.primary
    )
...
class TicketChannelModal(discord.ui.Modal, title="Ticket Message Channel Setup"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(
                "✅ Ticket message channel set!", ephemeral=True
            )
        except ValueError:
...
class SupportRoleModal(discord.ui.Modal, title="Support Role Setup"):
    ...
    role_id = discord.ui.TextInput(
        label="Role ID", placeholder="Enter the support role ID", required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if not category or not isinstance(category, discord.CategoryChannel):
                await interaction.response.send_message(
                    "❌ Invalid category!", ephemeral=True
                )
                return
...
class TicketCreateView(discord.ui.View):
    ...
    async def create_ticket(
        ...
        select = discord.ui.Select(
            ...
            options=[
                discord.SelectOption(label=category, emoji=emoji, value=category)
                for category, emoji in self.bot.config["tickets"]["categories"].items()
            ],
        )
        ...
        await interaction.response.send_message(
            "Please select a category:", view=view, ephemeral=True
        )
    ...
    async def create_ticket_channel(
        ...
        category_channel = interaction.guild.get_channel(
            self.bot.config["tickets"].get("category_id")
        )
        ...
        overwrites = {
            ...
            interaction.guild.get_role(
                self.bot.config["tickets"]["support_role_id"]
        ...
        message = await channel.send(
            content=f"{interaction.user.mention}", embed=embed, view=view
        )
        await message.pin()
...
class TicketControlView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Close", style=discord.ButtonStyle.danger, custom_id="close_ticket"
    )
    async def close_ticket(
        ...
        is_support = (
            interaction.user.get_role(self.bot.config["tickets"]["support_role_id"])
            is not None
        )
        is_owner = False
...
class ClosedTicketView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Reopen", style=discord.ButtonStyle.primary, custom_id="reopen_ticket"
    )
    async def reopen_ticket(
        ...
        if not interaction.user.get_role(self.bot.config["tickets"]["support_role_id"]):
            await interaction.response.send_message(
                "❌ Only support can reopen tickets!", ephemeral=True
            )
            return
        ...
        for user_id, ticket in self.bot.config["tickets"]["closed_tickets"].items():
            if ticket["channel_id"] == interaction.channel.id:
                ...
                if self.bot.config["logs"]["ticket"]["events"][
                    "ticket_close"
                ]:  # On utilise le même événement que la fermeture
                    self.bot.log_to_file(
                        "TICKET_REOPEN",
    ...
    async def create_transcript(self, channel: discord.TextChannel) -> discord.File:
        ...
        async for message in channel.history(limit=None, oldest_first=True):
            if (
                message.author.bot and not message.embeds
            ):  # Ignorer les messages du bot sauf les embeds
                continue
            ...
            if message.embeds:
                for embed in message.embeds:
                    messages.append(
                        f"[{timestamp}] [EMBED] {embed.title}: {embed.description}"
                    )
            else:
                messages.append(
                    f"[{timestamp}] {message.author.name}#{message.author.discriminator}: {message.content}"
                )

        transcript_text = "\n".join(messages)
        transcript_file = discord.File(
            fp=io.StringIO(transcript_text), filename=f"transcript-{channel.name}.txt"
        )
        return transcript_file

    @discord.ui.button(
        label="Delete", style=discord.ButtonStyle.danger, custom_id="delete_ticket"
    )
    async def delete_ticket(
        ...
        if not interaction.user.get_role(self.bot.config["tickets"]["support_role_id"]):
            await interaction.response.send_message(
                "❌ Only support can delete tickets!", ephemeral=True
            )
            return
        ...
        if self.bot.config["tickets"]["transcript_channel_id"]:
            transcript_channel = interaction.guild.get_channel(
                self.bot.config["tickets"]["transcript_channel_id"]
            )
            if transcript_channel:
                # Trouver les informations du ticket
                ticket_info = None
                ticket_owner = None
                for user_id, ticket in self.bot.config["tickets"][
                    "closed_tickets"
                ].items():
                    if ticket["channel_id"] == interaction.channel.id:
                        ticket_info = ticket
        ...
        await interaction.response.send_message(
            "Channel will be deleted in 5 seconds...", ephemeral=True
        )
...
class CategoryManageView(discord.ui.View):
    ...
    async def remove_category(
        ...
        async def select_callback(interaction: discord.Interaction):
            ...
            self.bot.save_config()
            await interaction.response.send_message(
                f"✅ Category '{category}' removed!", ephemeral=True
            )
...
class AddCategoryModal(discord.ui.Modal, title="Add Ticket Category"):
    ...
    category_name = discord.ui.TextInput(
        label="Category Name", placeholder="e.g. Support, Bug Report", required=True
    )

    category_emoji = discord.ui.TextInput(
        label="Category Emoji", placeholder="e.g. 🎮, 🐛", required=True, max_length=2
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if name in self.bot.config["tickets"]["categories"]:
            await interaction.response.send_message(
                "❌ This category already exists!", ephemeral=True
            )
            return
        ...
        if self.bot.config["tickets"]["message_channel_id"]:
            channel = interaction.guild.get_channel(
                self.bot.config["tickets"]["message_channel_id"]
            )
            if channel:
                view = TicketCreateView(self.bot)
                embed = discord.Embed(
        ...
        await interaction.response.send_message(
            f"✅ Category '{name}' added with emoji {emoji}!", ephemeral=True
        )
...
class TranscriptChannelModal(discord.ui.Modal, title="Transcript Channel Setup"):
    ...
    channel_id = discord.ui.TextInput(
        label="Channel ID", placeholder="Enter the transcript channel ID", required=True
    )
    ...
    async def on_message(self, message: discord.Message):
        ...
        for user_id, ticket in self.bot.config["tickets"]["active_tickets"].items():
            if (
                ...
                and ticket["status"] == "waiting_description"
            ):
                ...
                support_role = message.guild.get_role(
                    self.bot.config["tickets"]["support_role_id"]
                )
                await message.channel.send(
                    f"{support_role.mention} New ticket needs attention!"
                )
                break
