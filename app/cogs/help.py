...
LOGS_PATH = "/home/app/logs.txt"
MESSAGES_PATH = "/home/app/messages.txt"
CHANGES_PATH = "/home/app/changes.txt"
...
class Help(commands.Cog):
    ...
    @app_commands.describe(category="Catégorie de commandes spécifique (optionnel)")
    async def help(
        ...
        categories = {
            "moderation": {
                ...
                "commands": [
                    ...
                    (
                        "/set-timeout-duration [minutes]",
                        "Définir la durée des timeouts",
                    ),
                ],
            },
            "voice": {
        ...
        if category and category.lower() in categories:
            ...
            embed = discord.Embed(
                title=f"{cat['name']} - Commandes", color=discord.Color.blue()
            )
            ...
            for cat_id, cat_data in categories.items():
                ...
                embed.add_field(
                    name=cat_data["name"], value=f"```{cmd_list}```", inline=False
                )

        await interaction.response.send_message(embed=embed, ephemeral=True)

    @app_commands.command(name="logs", description="Affiche les logs du bot")
    @app_commands.describe(
        type="Type de logs à afficher", lines="Nombre de lignes à afficher (défaut: 10)"
    )
    ...
    async def logs(
        self, interaction: discord.Interaction, type: str, lines: Optional[int] = 10
    ):
        # Vérifier les permissions
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(
                "❌ Vous devez être administrateur pour voir les logs!", ephemeral=True
            )
            return

        # Mapper les types de logs aux fichiers
        log_files = {
            "logs": (LOGS_PATH, "🛡️ Logs de Modération"),
            "messages": (MESSAGES_PATH, "💬 Logs des Messages"),
            "changes": (CHANGES_PATH, "⚙️ Logs des Changements"),
        }

        if type not in log_files:
            await interaction.response.send_message(
                "❌ Type de logs invalide!", ephemeral=True
            )
            return
        ...
        try:
            ...
            with open(filename, "r", encoding="utf-8") as f:
                ...
                if not last_lines:
                    await interaction.response.send_message(
                        "📝 Aucun log disponible pour le moment!", ephemeral=True
                    )
                    return
                ...
                embed.add_field(name="Logs", value=log_content, inline=False)
            ...
            await interaction.response.send_message(
                f"❌ Fichier de logs '{filename}' introuvable!", ephemeral=True
            )
        except Exception as e:
            await interaction.response.send_message(
                f"❌ Erreur lors de la lecture des logs: {str(e)}", ephemeral=True
            )
