...
class ModerationSetupView(discord.ui.View):
    ...
    def update_buttons(self):
        for child in self.children:
            if isinstance(child, discord.ui.Button) and child.custom_id == "toggle":
                child.style = discord.ButtonStyle.green if self.bot.config["moderation"]["enabled"] else discord.ButtonStyle.red
    ...
    @discord.ui.button(label="Moderation: Enabled", style=discord.ButtonStyle.green, custom_id="toggle")
    async def toggle_moderation(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.bot.config["moderation"]["enabled"] = not self.bot.config["moderation"]["enabled"]
...
class ModChannelModal(discord.ui.Modal, title="Moderation Channel Setup"):
    ...
    channel_id = discord.ui.TextInput(
        label="Channel ID",
        placeholder="Enter the moderation channel ID",
        required=True
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            self.bot.save_config()
            await interaction.response.send_message(f"✅ Moderation channel set to {channel.mention}!", ephemeral=True)
        except ValueError:
...
class TimeoutDurationModal(discord.ui.Modal, title="Timeout Duration Setup"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            self.bot.save_config()
            await interaction.response.send_message(f"✅ Timeout duration set to {minutes} minutes!", ephemeral=True)
        except ValueError:
...
class BanDurationModal(discord.ui.Modal, title="Ban Duration Setup"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            self.bot.save_config()
            await interaction.response.send_message(f"✅ Ban duration set to {days} days!", ephemeral=True)
        except ValueError:
            await interaction.response.send_message("❌ Invalid duration!", ephemeral=True)
...
class Moderation(commands.Cog):
    ...
    def save_message(self, message: discord.Message):
        ...
        channel_name = message.channel.name if isinstance(message.channel, (discord.TextChannel, discord.Thread)) else "DM"
        ...
        if message.embeds:
            embeds = [f"\n- Embed: {embed.title} - {embed.description}" for embed in message.embeds]
    ...
    async def request_moderation_action(self, user: discord.Member, action_type: str):
        ...
        if view.result is True:
            ...
            if action_type == "timeout":
                duration = timedelta(minutes=self.bot.config["moderation"]["timeout_duration"])
    ...
    async def on_message(self, message: discord.Message):
        ...
        if banned_word := self.check_message(message.content):
            ...
            try:
                ...
                embed.add_field(name="Banned Word", value=f"||{banned_word}||", inline=False)
    ...
    @app_commands.command(
        name="infractions",
        description="View infractions for a user"
    )
    @app_commands.default_permissions(manage_messages=True)
    async def view_infractions(self, interaction: discord.Interaction, user: discord.Member):
        user_id = str(user.id)
        if user_id not in self.infractions:
    ...
    async def clear_infractions(self, interaction: discord.Interaction, user: discord.Member):
        user_id = str(user.id)
        if user_id not in self.infractions:
            await interaction.response.send_message(f"No infractions found for {user.mention}", ephemeral=True)
            return
        ...
        self.infractions[user_id] = {
            "current_infractions": 0,
            "total_infractions": old_data["total_infractions"],  # Garder le total historique
            "current_timeouts": 0,
    ...
    async def add_banned_word(self, interaction: discord.Interaction, words: str):
        ...
        response = []
        if added_words:
            response.append(f"✅ Added {len(added_words)} word(s): `{', '.join(added_words)}`")
        if already_exists:
            response.append(f"ℹ️ {len(already_exists)} word(s) already banned: `{', '.join(already_exists)}`")
    ...
    async def list_banned_words(self, interaction: discord.Interaction):
        ...
        words = sorted(self.bot.config["moderation"]["banned_words"])  # Trier alphabétiquement
    ...
    @app_commands.command(
        ...
        description="Enable or disable word moderation"
    )
    @app_commands.default_permissions(administrator=True)
    async def toggle_moderation(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"✅ Word moderation is now {status}",
            ephemeral=True
        )
    ...
    @app_commands.default_permissions(administrator=True)
    async def set_timeout_duration(self, interaction: discord.Interaction, minutes: int):
        if minutes < 1:
            await interaction.response.send_message("❌ Duration must be at least 1 minute", ephemeral=True)
            return
        ...
        await interaction.response.send_message(
            f"✅ Default timeout duration set to {minutes} minutes",
            ephemeral=True
        )

    @app_commands.command(
        name="set-ban-duration",
        description="Set the default ban duration (in days)"
    )
    @app_commands.default_permissions(administrator=True)
    async def set_ban_duration(self, interaction: discord.Interaction, days: int):
        ...
        await interaction.response.send_message(
            f"✅ Default ban duration set to {days} days",
            ephemeral=True
        )
    ...
    async def set_mod_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        await interaction.response.send_message(
            f"✅ Moderation channel set to {channel.mention}",
            ephemeral=True
        )
    ...
    async def cog_load(self):
        ...
        try:
            ...
            print(f"✗ Erreur lors de la synchronisation des commandes de modération: {str(e)}")
