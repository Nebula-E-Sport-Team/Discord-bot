...
class SetupView(discord.ui.View):
        ...
        async def select_callback(interaction: discord.Interaction):
            if select.values[0] == "jtc":
                ...
                await interaction.response.edit_message(
                    content="Join to Create System Configuration", view=jtc_view
                )
            elif select.values[0] == "ticket":
                ticket_view = TicketSetupView(self.bot)
                await interaction.response.edit_message(
                    content="Ticket System Configuration", view=ticket_view
                )
    ...
    async def setup_logs(
        ...
        async def select_callback(interaction: discord.Interaction):
            if select.values[0] == "voice":
                ...
                await interaction.response.edit_message(
                    content="Voice Channels Logs Configuration", view=logs_view
                )
            elif select.values[0] == "ticket":
                logs_view = TicketLogsView(self.bot)
                await interaction.response.edit_message(
                    content="Tickets Logs Configuration", view=logs_view
                )
    ...
    async def setup_moderation(
        ...
        await interaction.response.edit_message(
            content="Moderation System Configuration", view=view
        )
    ...
    async def setup_autorole(
        ...
        await interaction.response.edit_message(
            content="Autorole System Configuration", view=view
        )
...
class VoiceLogsView(discord.ui.View):
            ...
            if isinstance(child, discord.ui.Button):
                if child.custom_id == "create":
                    child.style = (
                        ...
                        if events["channel_create"]
                        else discord.ButtonStyle.red
                    )
                elif child.custom_id == "delete":
                    child.style = (
                        discord.ButtonStyle.green
                        if events["channel_delete"]
                        else discord.ButtonStyle.red
                    )
                elif child.custom_id == "rename":
                    child.style = (
                        discord.ButtonStyle.green
                        if events["channel_rename"]
                        else discord.ButtonStyle.red
                    )

    @discord.ui.button(label="Channel Creation", custom_id="create")
    async def toggle_create(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        self.bot.config["logs"]["voice"]["events"][
            "channel_create"
        ] = not self.bot.config["logs"]["voice"]["events"]["channel_create"]
    ...
    async def toggle_delete(
        ...
        self.bot.config["logs"]["voice"]["events"][
            "channel_delete"
        ] = not self.bot.config["logs"]["voice"]["events"]["channel_delete"]
        self.bot.save_config()
        self.update_buttons()
        await interaction.response.edit_message(view=self)

    @discord.ui.button(label="Channel Rename", custom_id="rename")
    async def toggle_rename(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        self.bot.config["logs"]["voice"]["events"][
            "channel_rename"
        ] = not self.bot.config["logs"]["voice"]["events"]["channel_rename"]
        self.bot.save_config()
        self.update_buttons()
        await interaction.response.edit_message(view=self)
    ...
    async def setup_channel(
        ...
        if not self.bot.config["jtc"]["category_id"]:
            await interaction.response.send_message(
                "❌ Please set up the category first!", ephemeral=True
            )
            return
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if not category or not isinstance(category, discord.CategoryChannel):
                await interaction.response.send_message(
                    "❌ Catégorie invalide!", ephemeral=True
                )
                return
            ...
            self.bot.save_config()
            await interaction.response.send_message(
                f"✅ Catégorie configurée: {category.name}", ephemeral=True
            )
        except ValueError:
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            channel = await interaction.guild.create_voice_channel(
                name=self.channel_name.value, category=category
            )
            ...
            await interaction.response.send_message(
                "❌ Limite d'utilisateurs invalide!", ephemeral=True
            )
...
class TicketLogsView(discord.ui.View):
        ...
        for child in self.children:
            if isinstance(child, discord.ui.Button):
                if child.custom_id == "create":
                    child.style = (
                        discord.ButtonStyle.green
                        if events["ticket_create"]
                        else discord.ButtonStyle.red
                    )
                elif child.custom_id == "close":
                    child.style = (
                        discord.ButtonStyle.green
                        if events["ticket_close"]
                        else discord.ButtonStyle.red
                    )
                elif child.custom_id == "delete":
                    child.style = (
                        discord.ButtonStyle.green
                        if events["ticket_delete"]
                        else discord.ButtonStyle.red
                    )
    ...
    async def toggle_create(
        ...
        self.bot.config["logs"]["ticket"]["events"][
            "ticket_create"
        ] = not self.bot.config["logs"]["ticket"]["events"]["ticket_create"]
        self.bot.save_config()
        self.update_buttons()
        await interaction.response.edit_message(view=self)
    ...
    async def toggle_close(
        ...
        self.bot.config["logs"]["ticket"]["events"][
            "ticket_close"
        ] = not self.bot.config["logs"]["ticket"]["events"]["ticket_close"]
        self.bot.save_config()
        self.update_buttons()
        await interaction.response.edit_message(view=self)
    ...
    async def toggle_delete(
        ...
        self.bot.config["logs"]["ticket"]["events"][
            "ticket_delete"
        ] = not self.bot.config["logs"]["ticket"]["events"]["ticket_delete"]
        self.bot.save_config()
        self.update_buttons()
        await interaction.response.edit_message(view=self)
...
class ForumSetupModal(discord.ui.Modal, title="Configuration du Forum"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            self.bot.config["forum"]["links"] = self.bot.config["forum"].get(
                "links", {}
            )
            self.bot.save_config()

            await interaction.response.send_message(
                f"✅ Forum configuré: {forum.name}", ephemeral=True
            )
        except ValueError:
            await interaction.response.send_message("❌ ID invalide!", ephemeral=True)
...
class VoiceCreator(commands.Cog):
    ...
    async def setup(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            "🔧 Bot Configuration", view=view, ephemeral=True
        )

    def is_channel_owner(self, member: discord.Member, channel_id: str) -> bool:
        """Vérifie si le membre est le propriétaire du salon"""
        return (
            str(channel_id) in self.bot.config["jtc"]["created_channels"]
            and self.bot.config["jtc"]["created_channels"][str(channel_id)] == member.id
        )

    @app_commands.command(
        name="voc-mute", description="Mute users in your voice channel"
    )
    ...
    async def voc_mute(
        ...
        if user:
            if user not in channel.members:
                await interaction.response.send_message(
                    "❌ This user is not in your channel!", ephemeral=True
                )
                return
            await user.edit(mute=True)
            await interaction.response.send_message(
                f"✅ {user.mention} has been muted", ephemeral=True
            )
        else:
            ...
            await interaction.response.send_message(
                "✅ All users have been muted", ephemeral=True
            )

    @app_commands.command(
        name="voc-kick", description="Kick a user from your voice channel"
    )
    ...
    async def voc_kick(self, interaction: discord.Interaction, user: discord.Member):
        ...
        await interaction.response.send_message(
            f"✅ {user.mention} has been kicked from the channel", ephemeral=True
        )

    @app_commands.command(
        name="voc-ban", description="Ban a user from your voice channel"
    )
    ...
    async def voc_ban(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if user.id in self.bot.config["jtc"]["banned_users"][channel_id]:
            await interaction.response.send_message(
                "❌ This user is already banned!", ephemeral=True
            )
            return
        ...
        await interaction.response.send_message(
            f"✅ {user.mention} has been banned from the channel", ephemeral=True
        )

    @app_commands.command(name="voc-rename", description="Rename your voice channel")
    ...
    @app_commands.command(
        name="voc-limit", description="Set user limit for your voice channel"
    )
    ...
    async def voc_limit(self, interaction: discord.Interaction, limit: int):
        if limit < 0:
            await interaction.response.send_message(
                "❌ The limit cannot be negative!", ephemeral=True
            )
            return
    ...
    @app_commands.command(
        name="voc-close", description="Close your voice channel to new users"
    )
    async def voc_close(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            "✅ Channel is now closed to new users", ephemeral=True
        )

    @app_commands.command(
        name="voc-open", description="Open your voice channel to new users"
    )
    async def voc_open(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            "✅ Channel is now open to new users", ephemeral=True
        )
    ...
    async def on_voice_state_update(
        ...
        if after.channel and str(after.channel.id) in self.bot.config["jtc"].get(
            "banned_users", {}
        ):
            if (
                member.id
                in self.bot.config["jtc"]["banned_users"][str(after.channel.id)]
            ):
