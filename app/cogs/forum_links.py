...
class ForumLinks(commands.Cog):
    ...
    @app_commands.command(
        name="lier",
        description="Lier un salon au forum avec des tags spécifiques"
    )
    ...
    async def link_channel(
        ...
        self.bot.config["forum"]["links"][str(channel.id)] = {
            "tags": valid_tags
        }
    ...
    async def on_message(self, message: discord.Message):
        try:
            ...
            title = message.content.split("\n")[0][:100] if message.content else "Nouveau message"
