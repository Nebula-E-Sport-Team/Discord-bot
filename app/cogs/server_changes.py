...
class ServerChanges(commands.Cog):
    ...
    async def on_guild_channel_create(self, channel: discord.abc.GuildChannel):
        ...
        details = f"New {channel_type} created: {channel.name} (ID: {channel.id})"
    ...
    async def on_guild_channel_delete(self, channel: discord.abc.GuildChannel):
        ...
        details = (
            f"{channel_type.capitalize()} deleted: {channel.name} (ID: {channel.id})"
        )
        if not isinstance(channel, discord.CategoryChannel) and channel.category:
    ...
    async def on_guild_channel_update(
        ...
        if not isinstance(before, discord.CategoryChannel):
            if before.category != after.category:
                old_category = (
                    before.category.name if before.category else "No category"
                )
                new_category = after.category.name if after.category else "No category"
                changes.append(
                    f"Category changed from '{old_category}' to '{new_category}'"
                )
