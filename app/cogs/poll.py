...
class PollView(discord.ui.View):
    ...
    def get_results(self):
        ...
        percentages = {opt: (votes/total_votes)*100 for opt, votes in results.items()}
...
class Poll(commands.Cog):
    ...
    async def create_results_image(self, question: str, results: dict, percentages: dict):
        ...
        try:
            ...
            if len(poll_options) < 2:
                await interaction.response.send_message(
                    "❌ Vous devez spécifier au moins 2 options!",
                    ephemeral=True
                )
                return
            ...
            embed.add_field(
                name="Type de vote",
                value="Choix multiples autorisés" if multiple_choice else "Un seul choix possible",
                inline=True
            )
            embed.add_field(
                name="Durée",
    ...
    @app_commands.command(
        name="endpoll",
        description="Terminer un sondage manuellement"
    )
    @app_commands.describe(
        message_id="ID du message du sondage"
    )
    async def end_poll_command(
        self,
        interaction: discord.Interaction,
        message_id: str
    ):
        try:
            ...
            if message_id not in self.active_polls:
                await interaction.followup.send("❌ Sondage non trouvé!", ephemeral=True)
                return
            
            view = self.active_polls[message_id]
            if view.ended:
                await interaction.followup.send("❌ Ce sondage est déjà terminé!", ephemeral=True)
                return
            
            try:
                message = await interaction.channel.fetch_message(message_id)
            except discord.NotFound:
                await interaction.followup.send("❌ Message du sondage introuvable dans ce canal!", ephemeral=True)
            ...
            await interaction.followup.send("✅ Sondage terminé avec succès!", ephemeral=True)
            
        except ValueError:
            await interaction.followup.send("❌ Format d'ID invalide! Utilisez un nombre entier.", ephemeral=True)
